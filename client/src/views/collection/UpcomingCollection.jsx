...
const UpcomingCollection = ({ data, onSuccess }) => {
  ...
  // Filter data based on name and date
  const filteredData = data
    ? data
        .filter((pending) => pending.person_name?.toLowerCase().includes(searchQuery.toLowerCase()))
        .filter((pending) => moment(pending.collectionDate).isAfter(todayDate)) // Only future dates
    : [];
      ...
      <Col xs={12} md={12} lg={12} className="mb-4">
        <Card className="Recent-Users widget-focus-lg shadow-lg">
          <Card.Header className="d-flex justify-content-between mobile-flex-container align-items-center">
            <Card.Title as="h5">Upcoming Pending Collection</Card.Title>
            <Form.Control
              type="text"
